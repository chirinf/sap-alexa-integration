...
PASSWD = 'Aws2022.'
LANG = 'S'
SAPHOST= 'sap-erq-ap1.camanchaca.cl'
SAPINSTANCENUM = '00'
SAPCLIENT = '300'
SAPROUTER = '/H/sr.camanchaca.cl/W/CCAPWD'

WORKLIST = []
TASK_FILTER =  ["TS20000166" ]
PO_HEADER = { }
PO_ITEMS = []
oldest_wi = ''

conn = Connection(ashost=SAPHOST, sysnr=SAPINSTANCENUM, client=SAPCLIENT, user=USERID, passwd=PASSWD, saprouter=SAPROUTER)
...
def get_welcome_response():
    ...
    speech_output = "Bienvenido a tu bandeja de pedidos SAP de Camanchaca. " \
                    "¿Qué necesitas hacer?"
...
def get_ContarItems(intent, session):
    ...
    reprompt_text = None
    # Llamar BAPI
    result = conn.call("SAP_WAPI_CREATE_WORKLIST", USER=USERID, LANGUAGE=LANG, WORKLIST=WORKLIST, TASK_FILTER=TASK_FILTER)

    #Pregunta 1 - Cuantos items tengo por aprobar?
    wi_pendientes = len(result["WORKLIST"])
    print (wi_pendientes)

    speech_output = "Tienes " + str(wi_pendientes) + " pedidos pendientes por aprobar." \
                        ". ¿Necesitas algo más?."
    should_end_session = False
...
def get_detallepedido(intent, session):
    ...
    if session.get('attributes', {}) and "nro_pedido" in session.get('attributes', {}):
        ...
        ". por un monto total de " + str(int(monto_total)) + " pesos. Corresponde a la compra de " + str(int(cantidad)) + " " + material + \
...
def get_apruebapedido(intent, session):
    ...
    if session.get('attributes', {}) and "nro_pedido" in session.get('attributes', {}):
        nro_pedido = session['attributes']['nro_pedido']
        bapi_aprobar = conn.call("BAPI_PO_RELEASE", PURCHASEORDER=nro_pedido, PO_REL_CODE='J7', USE_EXCEPTIONS='X')
